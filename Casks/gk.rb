# This file was generated by GoReleaser. DO NOT EDIT.
cask "gk" do
  version "0.68.0"

  on_macos do
    on_intel do
      sha256 "9d705891d75c49278525f1caf912e12a767dfe7d87092c2d75d6abca330b4436"
      url "https://github.com/x-mesh/gk/releases/download/v#{version}/gk_darwin_amd64.tar.gz"
    end
    on_arm do
      sha256 "2c45251b3c30602cd111e5fe82b190f2f311061a7ea68f1c562ffc60289f3c89"
      url "https://github.com/x-mesh/gk/releases/download/v#{version}/gk_darwin_arm64.tar.gz"
    end
  end

  on_linux do
    on_intel do
      sha256 "a8bed712829a2f7bc70aa96e383a0d17bfb9f925a6552ca76a4f736bf2491061"
      url "https://github.com/x-mesh/gk/releases/download/v#{version}/gk_linux_amd64.tar.gz"
    end
    on_arm do
      sha256 "a090682f476812fc54f565df31957656f830de1e843cf3a9456f591cc6791ea4"
      url "https://github.com/x-mesh/gk/releases/download/v#{version}/gk_linux_arm64.tar.gz"
    end
  end

  name "gk"
  desc "Lightweight Go git helper CLI (pull, log, status, branch)"
  homepage "https://github.com/x-mesh/gk"

  livecheck do
    skip "Auto-generated on release."
  end

  binary "gk"

  postflight do
    system_command "/usr/bin/xattr", args: ["-dr", "com.apple.quarantine", "#{staged_path}/gk"]
  end

  # No zap stanza required

  caveats <<~EOS
    gk uses git under the hood. Ensure git >= 2.30 is installed.
  EOS
end
